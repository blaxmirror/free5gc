...
func SeperateEtafId(etafid string) (regionId, setId, ptrId string, err error) {
	if len(etafid) != 6 {
		err = fmt.Errorf("len of etafId[%s] != 6", etafid)
		return
	}
	// regionId: 16bits, setId: 10bits, ptrId: 6bits
	regionId = etafid[:2]
	byteArray, err1 := hex.DecodeString(etafid[2:])
	if err1 != nil {
		err = err1
		return
	}
